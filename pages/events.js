  ...
  if (props.events == "") {
    ...
    eventsDisplay = props.events.map((x, y) => (
      <div class="eventNote" id={"notice" + y}>
        <h1
          class="everythinghasamarginnote"
          style={{ color: "red" }}
          id={"title" + y}
        >
          {x.title} by <span id={"club" + y}>{x.club}</span>
        </h1>
        <br />
        <h1 class="everythinghasamarginnote" id={"date" + y}>
          {x.date}
        </h1>
        <br />
        <p class="everythinghasamarginnote" id={"description" + y}>
          {x.description}
        </p>
      </div>
    ));
    for (let x = 0; x < props.events.length; x++) {
      ...
      $(`#notice${x}`)
        .css("background-color", `rgba(${r},${g},${b},0.8)`)
        .css("opacity", "0.9");
    }
  }
  return <div id="displayNotes">{eventsDisplay}</div>;
};
